...
unpack_home_dir() {
    ...
    if [ "$IT_TASK_DISPLAY" = "" ]; then
        error_msg "unpack_home_dir() IT_TASK_DISPLAY must be defined" 1
    fi
    ...
    #  Param checks
    #
    # Some of the checks below are ignored when $IT_TASK_DISPLAY is 1 
    # ie just inforoming what will happen
    #
    [ "$username" = "" ] && error_msg "unpack_home_dir() no username given" 1
    [ ! "$IT_TASK_DISPLAY" -eq "1" ] && [ "$(grep -c ^"$username" /etc/passwd)" != "1" ] && error_msg "unpack_home_dir($username) - username not found in /etc/passwd" 1
    [ "$home_dir" = "" ] && error_msg "unpack_home_dir() no home_dir given" 1
    [ ! "$IT_TASK_DISPLAY" -eq 1 ] && [ ! -d "$home_dir" ] && error_msg "unpack_home_dir($username, $home_dir) - home_dir does not exist" 1
    [ ! "$IT_TASK_DISPLAY" -eq 1 ] && [ "$(find "$home_dir" -maxdepth 0 -user "$username")" = "" ] && error_msg "unpack_home_dir($username, $home_dir) - username does not own home_dir" 1
